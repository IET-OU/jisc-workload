...
function calculateSummary($summaryRow) {
    ...
    var $row = $summaryRow.next();
    while($row.length === 1 && !$row.hasClass('summary')) {
        var total = 0;

        var wordcount = parseInt($row.find('input.wordcount').val(), 10);
        var studySpeed = parseInt($row.find('select').val(), 10);
        var av = parseInt($row.find('input.av').val(), 10);
        var other = parseInt($row.find('input.other').val(), 10);
        var FHI = parseInt($row.find('input.FHI').val(), 10);
        var communication = parseInt($row.find('input.communication').val(), 10);
        var productive = parseInt($row.find('input.productive').val(), 10);
        var experiential = parseInt($row.find('input.experiential').val(), 10);
        var interactive = parseInt($row.find('input.interactive').val(), 10);
        var assessment = parseInt($row.find('input.assessment').val(), 10);
        var tuition = parseInt($row.find('input.tuition').val(), 10);
    ...
    calculateSummaryTexts($summaryRow, totals);
}

function calculateSummaryTexts($summaryRow, totals) {
...
$(document).ready(function (e) {
        ...
        $('a.confirm-changes').on('click', function(e) {
            ...
            if(unsavedChanges === true) {
                result = confirm('You have unsaved changes. Click cancel to stay on the page or OK to continue without saving your changes');
            }
            if(result === true) {
        ...
        $('body').on('mouseover', '.popup', function(e) {
            ...
            if($(document).scrollTop() < $('.workload-table').offset().top) {
                height += $('.workload-table').offset().top - $(document).scrollTop();
            }
        ...
        var $fixedTable = $('<div style="display:none; overflow:hidden;position:fixed;"><table class="fixed-table"></table></div>');
        ...
        $('body').on('change', '#workload-table input, #workload-table select', function(e) {
            ...
            if(!$(this).hasClass('title')) {
                ...
                while(!$summaryRow.hasClass('summary')) {
                    $summaryRow = $summaryRow.prev();
                }
        ...
        $('body').on('click', '.remove-row', function(e) {
            ...
            if($row.next().length > 0 && !$row.next().hasClass('summary')) {
                ...
                if(itemId !== '') {
                    if($('#deleted-items').val() === '') {
                        $('#deleted-items').val(itemId);
                    }
                    else {
                        $('#deleted-items').val($('#deleted-items').val() + ',' + itemId);
                    }
                }
                $summaryRow = $row.prev();
                while(!$summaryRow.hasClass('summary')) {
